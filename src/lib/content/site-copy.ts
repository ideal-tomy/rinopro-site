...
export const aboutCopy = {
  hero: {
    ...
    sub:
      "東京都目黒区を拠点に、戦略コンサルティングの視点とAI開発の実装力を統合し、企画で止まらないDXを支援します。",
  },
  founding: {
    kicker: "OUR STORY",
    heading: "創業の経緯と問題意識",
    paragraphs: [
      "DXや生成AIが実用域に入った一方で、現場では「戦略資料はあるが動くものがない」「PoCで止まり本実装に進めない」というギャップが残っています。AXEONは、そのギャップを埋めるために、戦略コンサルティングで培った翻訳力と、AIを前提とした実装のスピードを同じチームで届けることを選びました。",
      "代表は戦略ファームにおいて製造・金融・流通・医療など幅広い業界の変革プロジェクトに関わり、「机上の戦略」と「現場で回る実装」のズレを何度も見てきました。技術責任者は大規模AIシステムの開発・運用に深く関与し、精度だけでなく運用コストとガバナンスまで見据えた設計を担ってきました。",
  ...
  background: {
    ...
    paragraphs: [
      "私たちは、戦略コンサルティングファームでの企業変革プロジェクトと、大手企業向けのAIシステム開発、その両方の現場で実務経験を重ねてきた少数精鋭です。",
