...
export function buildEmpathyLineA(path: FlowSelection[]): string {
  ...
  const byBuild: Record<string, string> = {
    ...
    build_chatbot:
      "難しく考えなくて大丈夫です。社内の質問・ナレッジをチャットで扱う場合も、最初から全情報を載せるより「参照が集中する場面」を1つに絞ると、現場で実際に使われる形になりやすくなります。例えば問い合わせの一次回答や社内ルール確認など、目的を明確にすると回答品質も安定します。まずは小さく始めて、使われ方を見ながら範囲を広げる進め方が安心です。",
    build_inquiry:
      "顧客向けの問い合わせ・受付を仕組み化する場合も、最初から完璧を目指さなくて問題ありません。まずは一次受付だけを整えるだけでも、担当者の負荷は大きく下がります。よくある質問の振り分けや必要情報の回収を先に自動化すると、後段の対応品質が安定しやすくなります。例外対応は人が見る前提で設計しておくと、安心感を保ちながら段階的に改善していけます。",
    build_platform:
...
export function buildEmpathyLineE(path: FlowSelection[]): string {
  ...
  const byChoice: Record<string, string> = {
    e_talk:
      "口頭やチャットで整理する進め方でも問題ありません。ただし次の段階に進むには、完璧な要件書ではなく「何を良くしたいか」が一文で伝わると、確認すべき点や進め方がはっきりします。いま分かる範囲から短く言語化し、不足は詳細見積のヒアリングやお問い合わせ前の確認でそろえていく形が近道です。",
    e_vague:
