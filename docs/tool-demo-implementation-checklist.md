...
## 6.5. デモ品質ゲート（量産時のチェックリスト）

新規デモ追加時に、以下を満たすこと。

- [ ] **30秒で理解できる構成**: 入力1回 → 自動生成 → 使える成果物2点の3ステップ
- [ ] **サンプル入力3件**: `sampleData` に実演用3パターンを設定
- [ ] **CTAが試作依頼に接続**: `ctaTitle` / `ctaButtonText` で「3営業日で試作」など問い合わせ導線を明示
- [ ] **出力は2点固定**: `outputStructure` で見出しを固定し、毎回同じ体裁で出力される
- [ ] **専門用語を避ける**: systemPrompt 内で「AI」「RAG」より「探す時間がなくなる」「文章作成が3秒」を使う

### モック→実AI 昇格ルール

新規デモは原則 **mock_preview** で投入し、反応の良いものだけ **ai_live** へ昇格する。

- [ ] **昇格指標**: クリック率・完走率・問い合わせ率のいずれかが閾値を超えたデモを候補とする
- [ ] **昇格手順**: Sanity で `runMode` を `ai_live` に変更し、`systemPrompt` を十分に設定してから Publish
- [ ] **詳細**: `docs/新規デモ追加手順.md` の「モック→実AI 昇格ルール」を参照
