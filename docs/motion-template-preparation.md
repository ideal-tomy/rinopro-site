...
**目的**: rinopro の「ページ遷移クロスフェード」「共通モーション定数・variants」「軽いプリミティブ」を、**新規案件ですぐ使える** GitHub Template リポジトリとして切り出す。
...
2. **ローカルで Next.js の最小アプリを用意**（`create-next-app` 推奨）し、rinopro から **決め打ちのファイルだけコピー**する。
3. **レイアウトをテンプレ用に最小化**（rinopro の `Header` / `Chat` 等は載せない）。
...
**やらないこと（初版）**: rinopro 全体の mirror、見積・チャット・Sanity 等のドメインコード、レイヤー2の先行実装。
...
- rinopro リポジトリを手元に clone 済みであること（コピー元）。
...
- **App Router + `src/`**: rinopro と同じ `@/*` → `./src/*` のパス解決に揃えやすい。
- **Tailwind**: rinopro の `globals.css` / ユーティリティ前提に合わせる（Tailwind v4 か v3 かは create-next-app の出力に合わせ、下記「手順 4」で調整）。

---

## 手順 3: rinopro からコピーするファイル（レイヤー0〜1・チェックリスト）

以下は **rinopro のパスをそのまま** `next-motion-template/src/` 配下へコピーする想定。
...
rinopro の `src/app/globals.css` は **プロジェクト全体のトークン・レイヤーが大きい**ため、テンプレでは次のどちらかでよい。

### 方針 A（推奨・手早い）

- create-next-app が生成した `src/app/globals.css` をベースにしつつ、rinopro から **次のブロックだけ**追記する。

  - `@media (prefers-reduced-motion: no-preference)` 内の  
    `::view-transition-old(root)` / `::view-transition-new(root)` のアニメーション指定  
    （rinopro の該当コメント付きブロック）

- そのうえで **ビルドエラーが出る Tailwind v4 / v3 の差**があれば、テンプレ側の公式構成に合わせて `@import` や `@layer` を調整する。

### 方針 B（rinopro に完全寄せ）

- rinopro の `globals.css` をほぼそのまま持ち込む。  
...
rinopro の `src/app/layout.tsx` は `Header` / `Footer` / `ChatContainer` 等に依存するため、**テンプレでは載せない**。
...
テンプレの `package.json` に、少なくとも次が含まれること（rinopro から版本はテンプレ生成時点の安定版に合わせてよい）。
...
rinopro にある他依存（Sanity, AI SDK 等）は **テンプレには入れない**。
...
*最終更新: rinopro 用メモ。テンプレリポジトリ側にも短い README を置くと、未来の自分が助かる。*
