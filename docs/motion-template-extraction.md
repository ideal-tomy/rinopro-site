...
rinopro ほぼ完成後のリファクタに向け、**再利用テンプレ（GitHub）に載せる単位**と **アプリに残すドメイン境界**をファイル単位で整理する。
...
| 2 | **同一 monorepo の `packages/motion-ui`（任意）** | レイヤー 0〜2 を npm パッケージ化 | rinopro を動かしたまま、他リポから `workspace:*` で共有 |
...
レイヤー 3  product / domain       → 見積フォーム、コンシェルジュ、デモ個別（rinopro に残す）
```
...
## 6. レイヤー 3 — rinopro に残す（テンプレにはサンプルのみ）
